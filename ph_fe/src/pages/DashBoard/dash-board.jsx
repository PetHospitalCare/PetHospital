                ...
                {isLoading ? (
                    ...
                    <>
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                            <StatCard
                                title={`Tổng lịch hẹn ${getTimeRangeText(timeRange)}`}
                                timeRange={timeRange}
                            ...
                            />
                            <StatCard
                                title={`Doanh thu ${getTimeRangeText(timeRange)}`}
                                value={stats.summary.revenueThisPeriod}
                                subValue={`${timeRange === "all" ? "Tổng doanh thu" : `Doanh thu ${getTimeRangeText(timeRange)}`}`}
                                icon="dollar"
                                isCurrency
                            />
                            <StatCard
                                title={`Thú cưng mới ${getTimeRangeText(timeRange)}`}
                                value={stats.summary.totalPets}
                                subValue={`${stats.details.pets.byType.dog || 0} chó, ${stats.details.pets.byType.cat || 0} mèo`}
                                icon="pet"
                            />
                            <StatCard
                                title={`Tài khoản mới ${getTimeRangeText(timeRange)}`}
